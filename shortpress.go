...
func compress(in string, inlen int, out *string, outlen int) int {
	var h1, h2, h3 uint
	verblen := 0
	_outlen := outlen
	verb := make([]string, 256)
	_out := &out
	for inlen != 0 {
		j := 7
		var needed int
		var flush, slot *string
		flush = nil
		h1 = uint(in[0] << 3)
		h2 = uint(in[0] << 3)
		if inlen > 1 {
...
func main() {
	for index := 0; index < len(Smazcb); index++ {
		fmt.Printf("%s s ", Smazcb[index])
	}
}
